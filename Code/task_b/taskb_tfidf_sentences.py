...
data = pd.DataFrame(dataset['test']).head(100)
...
for sent in data['sentence1']:
    words = sent.split(" ")
    for word in words:
        if word not in word_frequency:
            word_frequency[word] = 0

for sent in data['sentence2']:
    words = sent.split(" ")
    for word in words:
        if word not in word_frequency:
            word_frequency[word] = 0
...
total_docs=len(data['sentence1'])*2
...
file_path_output="sentences_output_tfidf.txt"
